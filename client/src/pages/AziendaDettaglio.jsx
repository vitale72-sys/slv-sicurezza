    ...
    <div className="page">
      ...
      {!['near_miss','whistleblowing','quiz'].includes(tab) && <div className="card">
        ...
        <div className="table-wrap">
          {tab==='lavoratori' && <TableLavoratori rows={rows} isAdmin={isAdmin} onEdit={r=>{setForm({...r});setModal('add');}} onDelete={handleDelete} onBadge={openBadge} />}
        ...
        <div className="modal-overlay" onClick={()=>setBadgeLav(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body" style={{textAlign:'center'}}>
              ...
              <p style={{fontSize:13,color:'#6b7280',marginTop:14}}>URL: <a href={badgeLav.url} target="_blank" rel="noreferrer" style={{color:'#0f3460'}}>{badgeLav.url}</a></p>
            </div>
            <div className="modal-footer">
    ...
    <tbody>{rows.length===0?<EmptyRow cols={6}/>:rows.map(r=><tr key={r.id}>
      ...
      <td>
          <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            <Actions row={r} isAdmin={isAdmin} onEdit={onEdit} onDelete={onDelete}/>
            {onBadge && <button className="btn btn-success btn-sm" onClick={()=>onBadge(r)}>QR Badge</button>}
          </div>
        </td>
    </tr>)}</tbody></table>;
}
